	...
	for{
		...
		if err != nil {
		   ...
		   if typeOnly.Type == "content" {
			   // Debug: log all clients in session
			   manager.Mutex.RLock()
			   clientCount := len(manager.Clients)
			   ...
			   manager.Mutex.RUnlock()
			   log.Printf("[DEBUG] Session %s has %d clients: %v", client.SessionID, clientCount, clientIDs)
			   ...
			   log.Printf("[DEBUG] Received content from %s (user: %s): content length=%d, position=(%f,%f)",
			   ...
			   for c := range manager.Clients {
				   if c != client {
					   select {
					   case c.Send <- message:
						   log.Printf("[DEBUG] Sent content to client %s", c.ID)
					   default:
						   log.Printf("[DEBUG] Client %s send channel full, skipping", c.ID)
					   }
				   }
			   }
			   manager.Mutex.RUnlock()
